...
@author: Fab
modified by BL to be compatible with '1) Detection Reconstruction Speedfilter'
'''
...
class Animal():
    ...
    def __init__(self, baseId, RFID, name=None, genotype=None, user1=None, age=None, sex=None, strain=None, setup=None,
    ...
    def loadDetection(self, start=None, end=None, lightLoad=False):
        ...
        if lightLoad == True:
            ...
            query = "SELECT FRAMENUMBER, MASS_X, MASS_Y, MASS_Z, FRONT_X, FRONT_Y, FRONT_Z, BACK_X, BACK_Y, BACK_Z,REARING,LOOK_UP,LOOK_DOWN FROM DETECTION WHERE ANIMALID={}".format(
                self.baseId)
        ...
        for row in rows:
            ...
            if not lightLoad:
                ...
                detection = Detection(massX, massY, massZ, frontX, frontY, frontZ, backX, backY, backZ, rearing, lookUp,
                                      lookDown)
            else:
        ...
        print(self.__str__(), " ", len(self.detectionDictionary),
    ...
    def filterDetectionByInstantSpeed(self, minSpeed, maxSpeed):
        """
        speed is in cm/s. We check consecutive frames (t vs t+1).
        """
        nbRemoved = 0
        keys_sorted = sorted(self.detectionDictionary.keys())
        for key in keys_sorted:
            ...
            # Convert to cm/s
            speed = (math.hypot(a.massX - b.massX, a.massY - b.massY)
                     * self.parameters.scaleFactor / (1 / 30))

            if (speed > maxSpeed) or (speed < minSpeed):
                self.detectionDictionary.pop(key, None)
                nbRemoved += 1

        print(f"Filtering Instant speed min:{minSpeed}, max:{maxSpeed}, removed:{nbRemoved}")
        #ADDED to original version
    def filterStationaryPeriod(self, framesForOneMin=1800, distanceThreshold=1):
        """
       removes frames which are part of a consecutive block of >= framesForOneMin
        with movement < distanceThreshold (in pixels) between consecutive frames.
        framesForOneMin = number of consecutive frames (default 1800 = 1 min @ 30 fps)
        distanceThreshold = movement threshold in pixels (default = 1 px)
        """
        keys_sorted = sorted(self.detectionDictionary.keys())
        ...
        for i in range(1, len(keys_sorted)):
            prev_frame = keys_sorted[i - 1]
            curr_frame = keys_sorted[i]
            prev_det = self.detectionDictionary[prev_frame]
            curr_det = self.detectionDictionary[curr_frame]

            dist = math.hypot(curr_det.massX - prev_det.massX, curr_det.massY - prev_det.massY)

            if dist < distanceThreshold:
                stationary_count += 1
            else:
                if stationary_count >= framesForOneMin:
                    start_idx = i - stationary_count
                    for j in range(start_idx, i):
                        frames_to_remove.add(keys_sorted[j])
                stationary_count = 1
        ...
        for f in frames_to_remove:
            self.detectionDictionary.pop(f, None)

        print(f"[Stationary Filter] Animal {self.baseId} ({self.name}): "
              f"Removed {len(frames_to_remove)} frames (≥{framesForOneMin} consecutive).")
    ...
    def getDistanceSpecZone(self, tmin=0, tmax=None, xa=None, ya=None, xb=None, yb=None):
        ...
        keyList = sorted(
            self.detectionDictionary.keys())  # get the list of the frames where the animal has been detected
        ...
        for key in keyList:
            ...
            if (
                    a.massX < xa or a.massX > xb or a.massY < ya or a.massY > yb or b.massX < xa or b.massX > xb or b.massY < ya or b.massY > yb):
                # if the animal is not in the zone, then the distance is not computed
                continue

            if (math.hypot(a.massX - b.massX,
                           a.massY - b.massY) > 85.5):  # if the distance calculated between two frames is too large, discard
                continue

            distance += math.hypot(a.massX - b.massX,
                                   a.massY - b.massY)  # add the distance to the previously calculated distance
    ...
    def getDistanceTo(self, t, animalB):
        ...
        dist = math.hypot(self.detectionDictionary[t].massX - animalB.detectionDictionary[t].massX,
                          self.detectionDictionary[t].massY - animalB.detectionDictionary[t].massY)
    ...
    def getDistanceToPoint(self, t, xPoint, yPoint):
        ...
        if (math.hypot(self.detectionDictionary[t].massX - xPoint, self.detectionDictionary[
                                                                       t].massY - yPoint) > self.parameters.MAX_DISTANCE_THRESHOLD):  # if the distance calculated is too large, discard
            return None

        else:
            distanceToPoint = math.hypot(self.detectionDictionary[t].massX - xPoint,
    ...
    def getDistanceNoseToPoint(self, t, xPoint, yPoint):
        ...
        if (math.hypot(self.detectionDictionary[t].massX - xPoint, self.detectionDictionary[
            ...
            distanceNoseToPoint = math.hypot(self.detectionDictionary[t].frontX - xPoint,
                                             self.detectionDictionary[t].frontY - yPoint)
    ...
    def getSap(self, tmin=0, tmax=None, xa=None, ya=None, xb=None, yb=None):
        ...
        for key in keyList:
            ...
            if (
                    detection.massX < xa or detection.massX > xb or detection.massY < ya or detection.massY > yb or detection.massZ > self.medianBodyHeight):
                # print ( 2 )
                continue
            ...
            if (
                    detection.getBodySize() >= self.bodyThreshold and speed < self.parameters.SPEED_THRESHOLD_LOW and detection.massZ < self.medianBodyHeight):
                # print ( 4 )
...
class AnimalPool():
    ...
    def loadAnimals(self, conn):
        ...
        for row in rows:
            ...
            if (len(row) == 7):
                animal = Animal(row[0], row[1], name=row[2], genotype=row[3], age=row[4], sex=row[5], strain=row[6],
                                conn=conn)
            if (len(row) == 8):
                animal = Animal(row[0], row[1], name=row[2], genotype=row[3], age=row[4], sex=row[5], strain=row[6],
                                setup=row[7], conn=conn)
            if (len(row) == 9):
                animal = Animal(row[0], row[1], name=row[2], genotype=row[3], age=row[4], sex=row[5], strain=row[6],
                                setup=row[7], user1=row[8], conn=conn)
    ...
    def loadAnonymousDetection(self, start=None, end=None):
        ...
        print(len(self.anonymousDetection),
              " frames containing anonymous detections loaded in {} seconds.".format(chrono.getTimeInS()))
    ...
    def filterStationaryPeriod(self, framesForOneMin=1800, distanceThreshold=1):
        for animalId, animalObj in self.animalDictionary.items():
            before_count = len(animalObj.detectionDictionary)
            animalObj.filterStationaryPeriod(framesForOneMin, distanceThreshold)
            after_count = len(animalObj.detectionDictionary)
            removed_count = before_count - after_count
            print(f"  - Animal {animalId}: removed {removed_count} frames due to stationary filter.")

        print(f"[Stationary Filter] Applied to all animals (≥{framesForOneMin} frames; dist < {distanceThreshold}px).")
    ...
    def plotSensorData(self, sensor="TEMPERATURE", show=True, saveFile=None, minValue=0, autoNight=False, title=""):
        ...
        for nightEvent in nightTimeLine.eventList:
            ...
            ax.text((nightEvent.startFrame + nightEvent.endFrame) / 2, mean + std, "dark phase", fontsize=20,
                    ha='center')
        ...
        if autoNight:
            ...
            for autoNight in autoNightList:
                ...
                ax.text((autoNight[0] + autoNight[1]) / 2, mean, "auto dark phase", fontsize=20, ha='center',
                        color='red')
    ...
    def buildSensorData(self, file, show=False):
        ...
        self.plotSensorData(sensor="LIGHTVISIBLE", minValue=40, saveFile=file + "_log_light visible.pdf", show=show,
                            title=file)
        self.plotSensorData(sensor="LIGHTVISIBLEANDIR", minValue=50, saveFile=file + "_log_light visible and infra.pdf",
                            show=show, title=file)
    ...
    def getParticleDictionary(self, start, end):
        ...
        query = "SELECT FRAMENUMBER, NUMPARTICLE FROM FRAME WHERE FRAMENUMBER>={} AND FRAMENUMBER<={}".format(start,
                                                                                                              end)
    ...
    def getDetectionTable(self):
        ...
        * adds column to indicate detections in center region
        ...
        df["x_cm"] = (df.x - self.parameters.cornerCoordinatesOpenFieldArea[0][0]) / (
                    self.parameters.cornerCoordinatesOpenFieldArea[1][0] -
                    self.parameters.cornerCoordinatesOpenFieldArea[0][0]) * self.parameters.ARENA_SIZE
        df["y_cm"] = (df.y - self.parameters.cornerCoordinatesOpenFieldArea[1][1]) / (
                    self.parameters.cornerCoordinatesOpenFieldArea[2][1] -
                    self.parameters.cornerCoordinatesOpenFieldArea[1][1]) * self.parameters.ARENA_SIZE
